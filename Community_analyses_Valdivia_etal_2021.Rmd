...
DATA
```{r}
edna_reads<-read.csv("/Volumes/GoogleDrive/Mi unidad/Doctorado CICESE/eDNA/Valdivia_etal_2021_community_analyses/edna.csv",row.names = 1)
```

UVC
```{r}
data_UVC<-read.csv("/Volumes/GoogleDrive/Mi unidad/Doctorado CICESE/eDNA/Valdivia_etal_2021_community_analyses/uvc.csv",row.names = 1)
```

SPATIAL
```{r}
spa<-read.csv("/Volumes/GoogleDrive/Mi unidad/Doctorado CICESE/eDNA/Valdivia_etal_2021_community_analyses/spa.csv", row.names = 1)
```

ENVIRONMENTAL
```{r}
env<-read.csv("/Volumes/GoogleDrive/Mi unidad/Doctorado CICESE/eDNA/Valdivia_etal_2021_community_analyses/env.csv", row.names = 1)
```
...
# Observing our results we can see that groups have significantly different compositions (p<0.05). So our groups (north and central) dont present homogeneity among group dispersions (compositions differ)
...
```{r}
Sobs_m_r<-read.csv("/Volumes/GoogleDrive/Mi unidad/Doctorado CICESE/eDNA/Valdivia_etal_2021_community_analyses/Sobs_method_region.csv",header=TRUE, colClasses = c("factor","factor","factor","numeric"))
head(Sobs_m_r)
```
